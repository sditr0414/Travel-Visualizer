...
상단 버튼의 높이와 위치를 통일하고 설정 그룹의 호버 배경 강조를 없앴습니다. 마우스로 연 설정 설명은 이름에서 벗어나면 닫힙니다. 기록 사이의 공백은 앞뒤 이동수단과 거리·시간을 참고해 이동수단을 보완합니다. 화면에는 별도 추정 표시 없이 이동수단 이름과 아이콘을 보여줍니다. 판단 근거가 부족하면 앞뒤 구간 중 거리·속도·방향이 더 비슷한 이동수단을 표시하며, 기록되지 않은 속도는 표시하지 않습니다.

설정과 이동수단 표시 변경은 타입 검사·린트·빌드, 단위 테스트 174개와 데스크톱·모바일 브라우저 검사 34개를 통과했습니다. 환경별 중복 및 선택적 실제 파일 검사 6개는 건너뛰었습니다. 390px 모바일 화면과 데스크톱에서 설정 버튼 정렬·도움말 동작을 확인했으며, 두 감상 모드에서 이동수단 이름·아이콘·구간별 이동거리도 검사했습니다. 브라우저 에뮬레이션 결과이며 모바일 실기기 검증은 포함하지 않습니다.
