...
기기 갤러리는 Takeout용 파일 입력을 우회하지 않고 선택한 `File[]`을 로컬 전용 로더에 직접 전달합니다. JPEG 촬영시각/GPS 분석은 가능한 경우 Web Worker에서 수행하고, Worker를 사용할 수 없으면 작은 배치로 메인 스레드에 양보하면서 처리합니다. 촬영시각은 **EXIF → 카메라 파일명 → 파일 수정시각** 순서로 보완합니다. 가져오기 단계와 사진/영상 수는 설정 화면의 진행 패널에서 확인할 수 있습니다.

사진 여정 재생 중에는 지도와 미디어 영역을 계속 같은 구도로 유지합니다. 데스크톱에서는 왼쪽 지도·경로와 오른쪽 미디어 레일, 모바일에서는 위쪽 지도와 아래쪽 미디어 영역을 사용하며, 사진이 나타날 때마다 전체 지도와 분할 화면을 반복 전환하지 않습니다. 장소와 촬영시각은 미디어 아래 정보 영역에 표시하고, 사진 GPS가 있으면 현재 지도 데이터에서 가능한 범위의 `시 · 구/행정구역` 이름을 우선 찾습니다.

설정의 `영상 길이`는 **경로 이동 부분의 목표 길이**입니다. 사진 정지 시간과 영상 재생 시간이 추가되면 실제 전체 재생시간은 더 길어지며, 재생바에 `현재 진행 시간 / 실제 전체 시간`으로 표시됩니다. `설정 적용 · 경로 다시 분석`을 눌러도 기존 영상 길이는 새 날짜 범위의 허용값 안에서 유지됩니다.
