...
이번 UI 변경은 타입 검사·린트·빌드, 단위 테스트 145개, 데스크톱·모바일 브라우저 검사 27개와 실제 로컬 타임라인 검사 1개를 통과했습니다. 전체 브라우저 실행에서 환경별 중복 및 선택적 실제 파일 검사 5개는 건너뛰었으며, 실제 파일 검사는 별도로 실행했습니다. 390px 모바일 화면과 데스크톱에서 첫 화면·설정·도움말·오류 복구를 확인했습니다. 브라우저 에뮬레이션 결과이며 모바일 실기기 검증은 포함하지 않습니다.
