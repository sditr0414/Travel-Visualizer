...
6. 사진 여정 생성
7. `다음 실행에도 이 브라우저에서 사용`을 사용자가 체크한 경우에만 선택한 미리보기를 여행 날짜 범위별로 브라우저 IndexedDB에 저장
8. 저장을 허용한 여행은 다음 실행부터 Picker 없이 자동 복원

Google Photos의 현재 권한 모델에서는 로그인만으로 사용자의 전체 라이브러리를 날짜 검색해서 자동 가져올 수 없습니다. 따라서 **여행마다 최초 1회 Picker 승인**은 필요합니다. 일정 범위 밖 사진 필터링과, 사용자가 저장을 허용한 여행의 반복 불러오기는 앱이 자동으로 처리합니다.
...
Google Photos Picker의 `baseUrl`은 임시 URL이고 사진 바이트 요청에는 OAuth Bearer 토큰이 필요합니다. 앱은 Picker 세션이 살아 있는 동안 영상용 미리보기를 내려받고 다운로드가 끝난 뒤 세션을 정리합니다. 브라우저 저장을 사용자가 허용하지 않으면 사진은 현재 실행에서만 사용합니다. 사진은 이 프로젝트 서버나 GitHub 저장소에 업로드되지 않습니다.
