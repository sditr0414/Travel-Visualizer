...
설정과 이동수단 표시 변경은 타입 검사·린트·빌드, 단위 테스트 164개와 데스크톱·모바일 브라우저 검사 32개를 통과했습니다. 환경별 중복 및 선택적 실제 파일 검사 6개는 건너뛰었습니다. 390px 모바일 화면과 데스크톱에서 설정 버튼 정렬·도움말 동작을 확인했으며, 두 감상 모드에서 이동수단 이름과 아이콘도 검사했습니다. 브라우저 에뮬레이션 결과이며 모바일 실기기 검증은 포함하지 않습니다.
