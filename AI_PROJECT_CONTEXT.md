...
- `npm run verify`는 TypeScript typecheck, ESLint `--max-warnings=0`, **15개 test file / 78개 테스트**, production build까지 통과한다. `MediaJourneyPane.test.tsx`의 일부 비동기 scene transition 테스트에는 비실패 React `act(...)` 경고가 남아 있으나 런타임 코드를 왜곡해 숨기지 않았다.
...
- GitHub Actions는 Node 24 기반 `actions/checkout@v5`, `actions/setup-node@v5`를 사용하고 `npm run verify` → Chromium 설치 → `npm run test:e2e` 순서로 실행한다. 코드 기준 **PR run 419**에서 전 단계가 성공했다.
