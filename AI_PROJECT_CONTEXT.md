...
- 기본 지도 확대는 사용자 설정 `+0.7`이며, 사진 여정에서는 미디어 표시 여부와 무관하게 **기본 추가 확대 `PHOTO_JOURNEY_BASE_ZOOM_BOOST = 0.62`**를 적용한다. 비행기 이외 이동에서는 짧은 구간일수록 `1.08 * exp(-distanceKm / 20)` 형태의 추가 확대를 더해, 구간 경계에서 계단식으로 바뀌지 않고 거리에 따라 연속적으로 감소시킨다. 항공 구간은 이 추가 확대를 적용하지 않는다.
...
- GitHub Actions는 Node 24 기반 `actions/checkout@v5`, `actions/setup-node@v5`를 사용하고 `npm run verify` → Chromium 설치 → `npm run test:e2e` 순서로 실행한다. 코드 기준 **run 416**에서 전 단계가 성공했다.
