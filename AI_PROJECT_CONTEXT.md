...
- `App`/reducer: 발자취와 사진 여정은 별도 `PlayerController` 인스턴스와 별도 재생 커서를 사용한다. 모드 전환 시 현재 controller를 멈춘 뒤 대상 controller의 커서를 복원하며 한 모드의 재생·탐색이 다른 모드의 진행 위치를 변경하지 않는다. 미디어가 이미 연결된 상태에서 같은 Timeline을 재계획할 때는 미디어를 새 계획에 다시 매칭하되 현재 여정 탭을 강제로 바꾸지 않는다. 경로 재생 길이는 사용자가 슬라이더를 직접 조정했는지 별도 ref로 기억해, 자동 상태에서는 기간 변경 후 재계획 시 새 추천 길이를 다시 적용하고 수동 상태에서는 사용자의 값을 우선한다. 사진 여정 stop에는 사진/영상 외에 선택적으로 날짜 marker를 합치며, marker 길이도 전체 사진 여정 재생 시간에 포함한다.
- `PlayerController`: 프레임 보간, 항공 안정 줌, stop 스케줄 재매핑과 재생 시간 기준을 검토했다. 사진 여정의 기본 거리 줌은 `0.42 + 0.96 * exp(-distanceKm / 20)` 계열의 연속 곡선을 사용하고 FLIGHT는 제외한다. 실제 사진 stop 동안에는 `photoStopZoomBoost()`가 짧은 구간일수록 더 큰 추가 줌을 점진적으로 적용한다. 사진 stop에서 route time이 멈춰도 카메라 추종 시간은 `this.timeSec`을 사용하므로 사진 표시 중에도 줌이 계속 움직인다. 사진 종료 뒤에는 기존 smooth zoom이 경로 줌으로 복귀하며 방향 반전 hysteresis, 속도/가속도 제한, overshoot 방지 계약을 유지한다.
...
- `npm run verify`는 TypeScript typecheck, ESLint `--max-warnings=0`, **14개 test file / 72개 테스트**, production build까지 통과한다. `MediaJourneyPane.test.tsx`의 일부 비동기 scene transition 테스트에는 비실패 React `act(...)` 경고가 남아 있으나 런타임 코드를 왜곡해 숨기지 않았다.
- `npm run test:e2e`는 Chromium desktop/mobile에서 **9개 통과, 3개 의도적 skip**이다. skip은 desktop 전용 hover 테스트의 mobile 프로젝트 1개와 `REAL_TIMELINE_JSON`이 없는 CI의 실제 대용량 Timeline 검증 2개다. desktop/mobile의 계획→재생→일시정지→reset, 별도 여정 커서, 설정 geometry, 사진 모드 playback control 배치를 모두 검증한다.
- GitHub Actions는 Node 24 기반 `actions/checkout@v5`, `actions/setup-node@v5`를 사용하고 `npm run verify` → Chromium 설치 → `npm run test:e2e` 순서로 실행한다. 코드 기준 run 413에서 전 단계가 성공했다.
