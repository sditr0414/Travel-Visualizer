...
- 위치 라벨: 이동 출발·도착은 `src/map/city-label.ts`, 사진 위치는 `src/map/photo-place-label.ts`가 이미 로드된 벡터 타일의 place 데이터를 사용해 해석한다. 모든 사용자 표시 위치의 최대 세분도는 시/도시 → 구/ward이며 군·읍·면·동·리·町·村 등 더 작은 행정/생활권 단위는 출력하지 않는다. 외부 reverse-geocoding 서비스로 좌표를 보내지 않는다.
...
- `PlayerController`: 프레임 보간, 항공 안정 줌, stop 스케줄 재매핑과 재생 시간 기준을 검토했고 stop 변경 시 경로 위치 보존 로직을 추가했다. 최종 카메라 줌은 planner 출력에 사진용 거리 확대/항공 안정 줌을 적용한 뒤에도 route-time 기반 저속 추종을 거쳐 MapLibre로 전달하므로 이동수단 또는 사진 구간 경계의 갑작스러운 줌 변화를 완충한다.
- `camera-modes`/camera planner: 이동수단별 목표 view span과 AUTO/DAY/SEGMENT 의미는 유지하되, 최종 zoom trajectory에 더 긴 zoom-out preview, 비대칭 smoothing, 속도/가속도 제한과 방향 반전 hysteresis를 적용한다. 작은 반대 방향 변화는 짧게 유지되는 동안 이전 줌 방향을 유지하고, 충분히 지속되거나 큰 프레이밍 변화일 때만 방향을 전환한다.
