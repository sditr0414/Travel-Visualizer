...
- 사진 여정에서 별도 현재 장면 HUD는 표시하지 않는다. 사진·영상이 활성화되지 않은 이동 구간에는 이동 수단 픽토그램과 라벨, 날짜, 속도를 표시하며 재생바의 미디어 연결 완료 문구도 비운다. 지도에 시/도시급 출발·도착 라벨이 모두 확인될 때만 `출발 → 도착`을 표시하고 구·동·좌표는 이동 구간 라벨에 사용하지 않는다.
