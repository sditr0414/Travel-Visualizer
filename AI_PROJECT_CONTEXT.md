...
- `MediaJourneyPane`/media bridge: 짧은 이동 구간 사진 유지, keyed outgoing scene, 사진/픽토그램 전환 identity와 시간 정책을 검토했다. 현재/다음 4개 미디어를 bounded preload window로 준비하고, 이미지 decode 또는 영상 first-frame readiness 전에는 기존 장면을 유지해 빈 사진 프레임이 노출되지 않게 한다. 사진/영상 dissolve는 560~700ms, 이동 픽토그램은 180~420ms 범위의 overlap을 사용해 순간 교체를 피한다. 사진 위치 문자열은 원시 좌표를 노출하지 않으며 coarse place label만 사용하고 해석 실패 시 `알 수 없음`을 표시한다.
