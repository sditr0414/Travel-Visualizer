...
- 속도 옆의 총 이동거리는 선택한 여행 계획에서 화면에 표시하는 이동수단별로 합산합니다. `movementDistanceTotals`는 `hideRoute`·`visual-gap`·유효하지 않은 거리를 제외하고, 컨트롤러를 준비할 때 한 번 계산해 두 감상 모드에서 공유합니다. 경로 재생 진행률과 무관한 여행 기간 전체의 합계이며 전체 경로 화면에서는 숨깁니다.
