...
- 속도 옆에는 현재 개별 이동 구간의 거리를 표시합니다. 같은 이동수단의 별도 기록은 합산하지 않습니다. `movementDistances`는 UNKNOWN에서 보완한 구간을 같은 표시 수단의 인접 기록 중 거리·유효 속도·방향이 더 비슷한 한쪽에만 연결해 해당 거리까지 포함하고, 동점이면 이전 구간에 붙입니다. 보완 구간에서도 연결된 이동과 같은 거리를 보여주며 중복 합산하지 않습니다. `hideRoute`·유효하지 않은 거리는 제외합니다. 컨트롤러 준비 시 계산해 두 감상 모드에서 공유하고 전체 경로 화면에서는 숨깁니다.
