...
- 재생 컨트롤은 하단에 두되 사진 여정에서는 지도 영역 안에 배치한다. 데스크톱 재생 중에는 상단 바·접힌 여행 설정·현재 장면 HUD·하단 재생바를 하나의 playback chrome으로 취급해 함께 숨긴다. 상단 조작, 설정, HUD, 하단 reveal zone 또는 재생바 중 어느 한 곳에 포인터가 접근하거나 키보드 포커스가 오거나 설정 패널이 열리면 모든 playback chrome을 함께 복원하며, 터치 화면에서는 항상 표시한다.
- playback chrome의 숨김·복원은 opacity·position·blur의 짧은 easing 전환으로 처리하고, 버튼·선택·설정 패널·카드형 상태 UI도 hover/focus/press/open 상태에 절제된 이동·색·테두리·그림자 애니메이션을 사용한다. 모든 모션은 `prefers-reduced-motion`을 존중한다.
