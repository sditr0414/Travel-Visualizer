...
- 마지막 ‘여행 전체’ 화면에서는 속도를 숨기고 `총 n km`로 여정 전체의 거리를 표시합니다. `totalJourneyDistance`는 합산된 표시 문자열을 다시 더하지 않고 원래 각 구간과 보완 구간을 한 번씩 합산하며, 제외 항공 등 `hideRoute`와 유효하지 않은 거리는 빼고 계산합니다.
