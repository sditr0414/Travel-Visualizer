...
describe('App integration', () => {
  ...
  it('updates individual distance on seek and replan and shows the journey total for the overview', async () => {
    ...
    firstPlan.segments.push({ ...firstPlan.segments[0], index: 1, distanceMeters: 650,
      startVideoSec: 0.5, endVideoSec: 1.5, videoSec: 1 });
    firstPlan.frames = [firstFrame, { ...firstFrame, timeSec: 0.5, segmentIndex: 1 }, { ...outro, timeSec: 2 }];
    firstPlan.durationSec = 2;
    firstPlan.travelDurationSec = 1.5;
    firstPlan.outroStartSec = 1.5;
    ...
    const worker: TimelineWorkerPort = {
      ...
      plan: vi.fn().mockResolvedValueOnce({ trip: {}, plan: firstPlan }).mockResolvedValue({ trip: {}, plan: secondPlan }),
      cancel: vi.fn(), dispose: vi.fn()
    };
    render(<App workerClient={worker} />);
    fireEvent.change(screen.getByLabelText('시작할 타임라인 파일 열기'), { target: { files: [timelineFile()] } });
    await waitFor(() => expect(screen.getByLabelText('전체 여정 분석')).toBeInTheDocument());
    const openingSummary = screen.getByLabelText('전체 여정 분석').textContent;
    expect(screen.getByLabelText('전체 여정 분석').querySelector('.journey-summary-total')).toHaveTextContent('2.7 km');
    expect(screen.getByLabelText('재생 위치')).toHaveValue('0');
    expect(fakeMap.fitBounds).toHaveBeenCalled();
    fireEvent.click(screen.getByRole('button', { name: '재생' }));
    expect(screen.queryByLabelText('전체 여정 분석')).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: '일시정지' }));
    ...
    expect(screen.queryByText('2 km')).not.toBeInTheDocument();
    expect(screen.getByLabelText('전체 여정 분석').textContent).toBe(openingSummary);
    expect(screen.getByLabelText('전체 여정 분석')).not.toHaveTextContent('km/h');
    ...
    await waitFor(() => expect(screen.getByLabelText('전체 여정 분석').querySelector('.journey-summary-total')).toHaveTextContent('5 km'));
    expect(screen.queryByText('2 km')).not.toBeInTheDocument();
    fireEvent.change(screen.getByLabelText('재생 위치'), { target: { value: '1' } });
    expect(screen.getByLabelText('전체 여정 분석').querySelector('.journey-summary-total')).toHaveTextContent('5 km');
  });
  ...
  it('starts photo journeys with preview media and keeps the route tab after replanning', async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText('전체 여정 분석')).toBeInTheDocument());
    expect(screen.getByLabelText('전체 여정 분석')).toHaveTextContent('총 1일');
    expect(screen.getByLabelText('전체 여정 분석').querySelector('.journey-summary-total')).toHaveTextContent('2 km');
    expect(screen.queryByText('12 km/h')).not.toBeInTheDocument();
