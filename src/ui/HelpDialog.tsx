...
export function HelpDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  return <Dialog open={open} onClose={onClose} title="처음 사용하는 분을 위한 안내" subtitle="내 파일을 준비하면 여행을 바로 재생할 수 있어요.">
    ...
    <section className="help-section"><h3>두 가지 감상 방식</h3><p><strong>경로 보기</strong>는 이동 경로를 따라갑니다. <strong>사진 여정</strong>은 사진과 영상이 나올 때 경로를 잠시 멈춥니다. 두 모드는 각각 보던 위치를 기억합니다.</p></section>
    ...
    <section className="help-section"><h3>개인정보와 지도</h3><p>Timeline과 사진 원본은 외부로 업로드하지 않습니다. 온라인 지도는 배경 타일을 받기 위해 인터넷에 연결합니다. 설치형 지도는 필요한 한글 지명 글꼴을 최초 사용 시 받아 이 PC에 캐시합니다.</p><p>‘정확한 장소 온라인 확인’은 기본으로 꺼져 있습니다. 서버에 장소 서비스를 직접 설정하고 이 옵션을 켠 경우에만 신뢰 가능한 사진 GPS 좌표를 보내며, 사진 원본과 Timeline은 보내지 않습니다. 확인 결과는 같은 위치를 반복 조회하지 않도록 로컬 서버 캐시에 저장합니다.</p><p>감상 설정은 이 브라우저에 저장합니다. 직접 선택한 파일은 새로고침하면 다시 선택해야 합니다. PC에서 실행한 로컬 서버에 휴대폰이 자동으로 연결되지는 않습니다. 모바일 화면에서는 해당 기기의 파일을 사용하세요.</p></section>
